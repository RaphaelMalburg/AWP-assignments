...
type TickerMeta = { symbol: string; label: string };
...
// Accepts (shares_ignored, per_share_price) — first param is ignored
// Callers must pass (open, price); the second arg is what gets scaled.
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json({ success: true, data: payload, source: "yahoo" });
  } catch {
    const payload = TICKERS.map((t) => {
    ...
    return NextResponse.json({ success: true, data: payload, source: "mock" });
  }
}
